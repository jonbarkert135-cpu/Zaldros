...
**IMPLEMENTATION** — `spec/PART_4.md` preserved. New component `tools/zaldros-compat` v0.1.0: two
...
truthful state, since no Zaldros image has ever been booted.
...
**NEXT** — PART 5 integration, and, as soon as a build host exists: build `zaldros-base`, boot it in
QEMU, run `zaldros-sysprobe` + `zaldros-hwinfo` inside it and record the first evidence entries.
