# Zaldros OS — Development roadmap
...
| 3 | Zaldros Shell | not started |
| 4 | Taskbar + Start + Search | not started |
| 5 | Explorer | not started |
| 6 | Settings | not started |
| 7 | System utilities | backends started (`zaldros-sysprobe`, `zaldros-hwinfo`) |
| 8 | Application management | not started |
| 9 | Windows compatibility | registry started (`zaldros-compat`) |
| 10 | Installer + recovery | not started |
| 11 | Performance optimization | harness started (`zaldros-bench`) |
...
- [x] First tool: `zaldros-sysprobe` (service & resource map generator, spec §7)
...
- [x] Windows → Zaldros feature matrix (`docs/architecture/FEATURE_MATRIX.md`)
- [x] Benchmark harness `zaldros-bench` (PART 5 §1)
Gate: architecture documents reviewed + a reproducible baseline image builds in CI.

## Phase 1 — Bootable base image
- Containerfile for `zaldros-base` (core system, no shell) and `zaldros-desktop`
...
Gate: VM boots to a Plasma/Wayland session, `zaldros-sysprobe` report attached to the build.

## Phase 2 — Zaldros Desktop Shell v1
Taskbar (pinned/running/previews/tray/clock), Zaldros Start, global search, quick settings,
...
## Phase 3 — Zaldros System Applications
...
Wine/Proton integration, Zaldros Setup installer, driver/firmware handling, hardware test matrix.
