# Windows 11 ↔ Zaldros OS parity

(Requested as `WINDOWS_ZALDROS_PARITY.md`; renamed to match the official project name — see `docs/NAMING.md`.)
...
| Windows feature | Zaldros implementation | Status | Working? | Tested? | Visual | Behaviour | Known limitations | Priority |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
| Taskbar | Zaldros Taskbar (QML), Windows 11 metrics | PROTOTYPE | yes (rendered) | yes | 4.0 | 3 | window, not a layer-shell panel; no real window list | P1 |
| Start menu | Zaldros Start, 640×726, pins + all apps | PARTIAL | yes — launches real apps | yes | 4.0 | 3 | no search results, Recommended empty by design | P1 |
...
| Task Manager | `zaldros-sysprobe` backend | BACKEND ONLY | CLI only | yes (unit) | — | — | no GUI | P3 |
| Device Manager | `zaldros-hwinfo` backend | BACKEND ONLY | CLI only | yes (unit) | — | — | no GUI | P3 |
