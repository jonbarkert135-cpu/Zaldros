...
carries at least one evidence record with a date, a Bedrock build id and a tester. CI runs
``python -m bedrock_compat --check`` so an unevidenced claim fails the build instead of shipping.
"""
