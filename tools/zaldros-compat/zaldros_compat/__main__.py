"""CLI: validate the compatibility registries or render them as reports.
    ...
    python -m zaldros_compat --check                 # exit 1 if any claim lacks evidence
    python -m zaldros_compat --report hardware       # markdown report
"""
...
REGISTRIES = {
    "apps": (os.path.join(DATA_DIR, "applications.json"), "app", "Zaldros Linux — application compatibility"),
    "hardware": (os.path.join(DATA_DIR, "hardware.json"), "hardware", "Zaldros Linux — hardware compatibility matrix"),
}


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="zaldros-compat", description=__doc__)
