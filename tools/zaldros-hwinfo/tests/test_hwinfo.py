...
from zaldros_hwinfo.hwinfo import (  # noqa: E402
    collect,
    collect_batteries,
    collect_disks,
    collect_displays,
...
def test_parse_os_release_handles_quotes():
    name, version = parse_os_release('NAME="Zaldros Linux"\nVERSION_ID="0.1"\n')
    assert (name, version) == ("Zaldros Linux", "0.1")
...
def test_full_collect_and_reports(fake_sys, fake_proc):
    ...
    assert "# Zaldros Linux — system & device inventory" in markdown
