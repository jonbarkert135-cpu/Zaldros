...
## 2026-08-31 — Перетаскивание к кромке экрана: зона выбирается точкой отпускания мыши

Закрываю расхождение, которое сам зафиксировал в предыдущей записи: у нас зона выбиралась кликом по
макету, а в Windows — тем, где отпустили мышь. Теперь работает и второе, по описанию Microsoft
(«Drag a window to the left or right edge of the screen and release it to snap it to that half;
drag it into a corner to snap it to a quadrant»).

**Реализация.** `AppWindow` во время перетаскивания сообщает позицию курсора в координатах оболочки
(`dragPointMoved`) и факт отпускания (`dragDropped`) — решение о зоне принимает `Shell`, окно про
зоны ничего не знает. `Shell.edgeZoneAt(sx, sy)`: у левой/правой кромки (`Theme.edgeSnapMargin` = 8)
— соответствующая половина, в полосе угла (`Theme.edgeSnapCorner` = 120) — квадрант, вне кромок и
ниже рабочей области (над таскбаром) — `null`. Верхнюю кромку намеренно не трогаю: ею уже владеет
snap-бар. `reportDragPoint` запоминает, **чьё** это перетаскивание, поэтому отпускание другого окна
чужую зону не забирает (есть тест). Дроп идёт через тот же `applySnap`, значит Snap Assist после
кромочного снапа предлагает окна точно так же.

**Превью.** Прозрачная панель `edgePreview` (z 38, радиус 8, `Theme.edgePreviewFill/Border`) ровно на
месте будущей зоны — как серая плашка Windows. Оба числа (8 и 120) помечены как DERIVED: у Microsoft
они не опубликованы.

**Доказательства.** parity: два новых состояния `edgedrag` и `edgedragcorner`, 8 проверок →
**110/110**. Новый `tests/test_edge_drag.py` — 11 тестов. Полные гейты: shell **515 passed,
1 skipped**, tools **44 passed**. По-прежнему offscreen/QEMU: на реальном ASUS TUF F15 система ещё
не загружалась.

## 2026-08-31 — Snap assist: после снапа предлагаем остальные окна во вторую зону

Третья часть механики snap из Windows 11. По документации Microsoft
(support.microsoft.com/en-us/windows «Snap your windows»): после того как окно занято половину или
угол экрана, Windows показывает в оставшейся свободной области миниатюры остальных открытых окон,
и клик по одной из них ставит её рядом. Это и реализовано.

**Честность по эталону.** Подлинного снимка Microsoft именно этого экрана с **опубликованным
масштабом** в библиотеке нет, поэтому в `win11-reference.json` я осознанно **ничего не добавлял**.
Геометрия карточек выведена из уже доказанных величин самой оболочки (отступ 16, зазор 12, подпись 36)
и проверяется как соотношение, а не как измеренный пиксель: превью держит 16:9 (178±4 → фактически 177),
сетка отцентрована в свободной зоне.

**Реализация.** Новый `qml/SnapAssist.qml`: затемняющая пелена поверх свободной зоны + центрированная
сетка карточек (до 4 штук, 1 колонка при одном кандидате, иначе 2). Клик по карточке — `windowChosen`,
клик по пелене — `dismissed`. В `Shell.qml` появились `freeZone(zone)` (возвращает наибольшую свободную
полосу: для половины — противоположная половина, для угла — противоположная половина на всю высоту, для
полного экрана — ничего) и `assistCandidates(exclude)` (открытые, не заснапленные, кроме исходного).
Снап разделён на две функции с фиксированной арностью: `snapInto(id, zone)` — только снап,
`applySnap(id, zone)` — снап плюс предложение assist. Имена и глифы окон вынесены в общий `windowMeta`,
таскбар теперь читает их оттуда (строки те же, кадр попиксельно не изменился).

**Оговорка про поведение.** У нас зона выбирается кликом по макету, в Windows — точкой отпускания
мыши при перетаскивании. Assist после этого работает одинаково, но сам способ выбора зоны пока
отличается — это в TODO.

**Доказательства.** `tools/visual/parity.py`: новое состояние `snapassist`, 7 новых проверок →
**102/102**. Новый `tests/test_snap_assist.py` — 10 тестов. Полные гейты: shell **504 passed,
1 skipped**, tools **44 passed**. Всё это по-прежнему offscreen/QEMU — на реальном ASUS TUF F15
система ещё не загружалась.

## 2026-08-31 — Snap-бар: перетаскивание окна к верхней кромке (Windows 11)

Продолжение автономной работы над «как Windows 11 один к одному». После snap layouts следующая
честно измеримая часть той же механики — **snap-бар**: в Win11 при перетаскивании окна к верхней
кромке сверху выезжает полоса с теми же шестью макетами, подсказкой «Перетащите окно сюда…» и
значком Win+Z.

**Эталон и его границы.** В библиотеке лежит подлинный снимок Microsoft именно этой полосы
(`assets/refs/win11/cache/snap-bar1.png`, id `snap-bar-2025`). Но у него **не опубликован масштаб**,
и правую половину закрывает окно, поэтому его пиксели нельзя читать как логические. Что у двух
snap-снимков общее — полоса миниатюр. Поэтому новая функция `measure_snap_bar_hint()` нормирует
снимок по высоте полосы миниатюр (91 px против доказанных 62 при 100 %) и берёт из него только
вертикальные полосы: шапка 69 px → **47.0**, нижний отступ 20 px → **13.6**, боковой 18 px → **12.3**
(то есть тот же отступ 13, в пределах допуска). Всё это записано в `win11-reference.json` как
`snap_bar` с пометкой «DERIVED, NOT MEASURED IN LOGICAL PIXELS» — чтобы никто (включая меня в
будущем) не принял эти числа за измерение при 100 %. Горизонтальные числа — из флайаута, измеренного
при 100 %. Инструмент печатает **40/40**.

**Реализация.** `qml/SnapBar.qml` переиспользует `SnapLayouts` (у него появились `frameVisible`,
`namePrefix` и `topInset`), поэтому у бара и флайаута ровно один источник макетов — тест это
проверяет. Над полосой — иконка, подсказка и значок «⊞ + Z». `AppWindow` во время перетаскивания
(`drag.active`) сообщает `snapBarRequested(atTopEdge)`, когда окно поднялось выше
`Theme.snapBarTrigger` (24 px); `Shell.requestSnapBar()` открывает бар и помнит, **какое** окно
тянут, — клик по зоне снапит именно его, а не активное. Уход от кромки, снап и Esc бар закрывают.

**Доказательства.** `tools/visual/parity.py`: новое состояние `snapbar` и 9 проверок → **95/95**.
Новый `tests/test_snap_bar.py` — 7 тестов (размер из эталона, один набор макетов на два интерфейса,
открытие/закрытие, снап тянущегося окна, проводка всех пяти окон, шапка = 34 + 13 = 47). Гейты:
`494 passed, 1 skipped` (shell) и `44 passed` (tools). Кадр — `docs/visual/current/snapbar.png`.

**Что по-прежнему не доказано.** Всё это QEMU и офскрин-рендер. На железе владельца система ещё не
загружалась; в Win11 зона выбирается отпусканием мыши над ней, у нас — кликом (перенос настоящего
drop на зоны потребует настоящего оконного менеджера, а не наших пяти окон-макетов).

## 2026-08-31 — Snap Layouts: шесть макетов Windows 11, выведенных из снимка Microsoft

Владелец: «продолжай разработку автономно, чтобы система стала как Windows 11 один к одному, смотри
скриншоты в интернете». Самая заметная незакрытая фишка Win11 из `TODO.md` — snap layouts. Сделано
по нашему же процессу (`docs/VISUAL_PARITY_PROCESS.md`): сначала ЭТАЛОН, потом реализация.

**Эталон.** В библиотеке `assets/refs/win11/library.json` уже лежал подлинный снимок Microsoft со
snap-баром 22H2. Его масштаб доказывается им самим: таскбар на снимке 47 px против задокументированных
48, то есть это захват при 100 %, и пиксели на нём — логические. Измерено (`tools/visual/measure_library.py`,
новая функция `measure_snap_bar`): панель 672 px, шесть макетов 96x62, зазор между макетами 14,
между ячейками внутри макета 6, отступ панели 13. Дальше главное: **шесть макетов не выбраны, а
выведены** — ширины ячеек, поделённые на содержимое миниатюры, дают ровно 1/2+1/2, 2/3+1/3,
1/2 + две четверти, четыре квадранта, три трети и 1/4+1/2+1/4. Инструмент печатает 36/36 совпадений
с `win11-reference.json`, включая долю каждой колонки каждого макета.

**Реализация.** `qml/SnapLayouts.qml` — флайаут, который открывается под кнопкой «развернуть» после
400 мс наведения (Windows открывает его именно по задержке, а не по клику) и по Win+Z. Клик по зоне
уводит окно в эту долю рабочей области. Геометрия ячеек считается так же, как у Windows: зазор
вычитается из содержимого миниатюры, а ячейка, перекрывающая внутреннюю границу, забирает зазор себе —
поэтому половина в миниатюре 96 px шириной равна 45 px, а не 48, и полноразмерная левая ячейка рядом
с расколотой колонкой остаётся 62 px высотой. Состояние окон переехало в один узел: `winX/winY/
winWidth/winHeight` в `Shell.qml` (зона → развёрнуто → своя геометрия), так что пять окон больше не
повторяют одну и ту же тройную тернарную формулу. Привязанное окно не тащится мышью, как и
развёрнутое; двойной клик по заголовку из зоны выводит.

**Доказательства.** `tools/visual/parity.py` получил два новых состояния (`snap`, `snapped`) и 46
проверок: панель, миниатюры, зазоры и **каждая зона каждого макета** — все совпали с пикселями
ячеек на снимке Microsoft (45/45, 60/30, 45+28/28, квадранты 45x28, 28/28/28, 21/42/21), плюс
привязанное окно занимает ровно левую половину рабочей области (0,0 640x752 на 1280x800). Итог:
**86/86 проверок** визуального паритета. `tests/test_snap_layouts.py` прогоняет все 17 зон шести
макетов через тот же `applySnap`, которым пользуется флайаут.

**Заодно исправлено то, что мешало вообще что-либо проверить:** `hit_boxes()` искал элементы через
`findChild`, а элементы, созданные `Repeater`, не имеют QObject-родителя — их там нет и никогда не
было. Теперь обход идёт по визуальному дереву (`childItems`), и любой именованный делегат виден
и тестам, и гостевому UI-тесту.

Реальные окна приложений привязываются штатным quick tiling KWin (Meta+стрелки уже засеяны в
`kglobalshortcutsrc`); тест это состояние фиксирует, чтобы оно не пропало.
...
## 2026-08-28 — Доказательства обязаны переживать прогон (ADR-0024)

**Красный прогон при зелёной системе.** `iso` 33161289986: три сборки и восемь загрузок из девяти
прошли, а прогон красный — `boot (services, mid)` упал на публикации доказательств:
`cannot lock ref 'refs/heads/ci-logs-boot-services-mid'`. Два перекрывающихся прогона на main
force-push'ат одну ветку, проигравший получает отказ. Публикация вынесена в
`build/iso/publish-evidence.sh`: повторы (по умолчанию 5), конвертация PPM → PNG, метка прогона в
`RUN.txt`, а если ничего не опубликовалось — громкий ненулевой выход. Токен в лог не попадает:
пуш идёт в именованный remote, вывод фильтруется. Задание, ничего не собравшее, публикует ветку с
`NOTE.txt` — читателю нужен ответ.

**Диагностика, исчезавшая ровно когда нужна.** `build-iso.sh` снимал EXIT-трап перед `mksquashfs`
и `grub-mkrescue` — вся вторая половина сборки шла без диагностики. Из-за этого `environment.txt`
не появлялся даже при успехе, шаг «Diagnostics summary» падал с кодом 1 под `continue-on-error`, и
из summary вместе с ним пропадали последние 200 строк лога сборки. Трап больше не снимается;
внутри `collect_debug` стоит `set +e`, чтобы первый упавший зонд не обрывал файл на середине.

**Ложный FAILURE в конце каждой успешной сборки.** `xorriso … -find /EFI` спрашивал не то:
`grub-mkrescue` кладёт UEFI не каталогом `/EFI`, а FAT-образом `/efi.img` вторым образом каталога
El Torito. Новый `build/iso/verify-iso.py` читает `-report_el_torito plain` и отвечает кодом:
0 — UEFI-образ есть, 1 — каталог прочитан и образа нет (сборка падает, такой ISO не грузится на
UEFI), 3 — UNVERIFIED. Отчёт сохраняется в `el-torito.txt` и попадает в summary.

Гейты: shell 465 passed + 2 skipped, tools 44. Тесты проверяют факты, а не хост: публикация — на
настоящем локальном bare-репо и на заглушке `git`, проигрывающей гонку дважды; диагностика —
реальным запуском `build-iso.sh` без root (под root тест пропускается); вердикт UEFI — на
дословном отчёте xorriso из прогона 33161289986.

**Чего нет.** Живое железо по-прежнему не проверено, ISO не собран под настоящим root. `verify-iso.py`
...
## Прогон #41: переключатель наконец виден — и сразу косметика под Windows 11

Прогон #41 (iso `33326233414`, 12/12 зелёных) закрыл вопрос: `switcher_overlay_fraction: 0.99024`,
`switcher_visible: true`, `held_equals_after: false`, md5 held (`85ffc50…`) ≠ after (`6fa33c8…`),
в логе сессии `ZALDROS-SWITCHER tabbox surface visible=true geometry=0,0 1280x800`. То есть виноваты
были обе причины разом: скрипту KWin окна не дают, а в tabbox-layout'е стоял тот же дефект флагов.

Скриншот показал остальное: карточки лежали прямо на затемнённом рабочем столе. В Windows 11 они
собраны на одной скруглённой панели. Что изменено в `system/theme/tabbox/zaldros`:

- карточки помещены на панель (`id: panel`) цвета `@SURFACE@` с alpha 0.92, радиус `@RADIUS@ * 2`,
  рамка 1 px белого 8 %. Токен `surfaceColour` до этого объявлялся и не использовался;
- карточки крупнее: 300×188 вместо 280×175, зазор 16 вместо 12, отступ панели 20;
- выделение — заливка белым 16 % и рамка акцентом в 2 px (было 10 % и 1 px);
- миниатюра сидит в держателе со скруглением карточки. Углы самой миниатюры остаются прямыми:
  `clip` в Qt прямоугольный, а маска-шейдер этого не стоит.

Настоящего acrylic-размытия здесь не будет: оно тянет за собой Plasma QML stack, которого в сессии
Zaldros нет (ADR-0008). Панель тонированная, не размытая — это сознательный размен.

Гейты: shell 474 passed, 1 skipped; tools 44 passed.

### Матрица результатов падала на своих же диагностиках

Слитый в main прогон (iso `33326841932`) сделал красными все boot-джобы на шаге «Result matrix»,
хотя сама загрузка проходила: в `results/` теперь остаётся `*.late.json` (диагностика из гостя,
ADR-0024), а `report.py` считал её ещё одним образом и печатал строку `| ? | ? | FAIL … |`,
из-за которой отчёт возвращал 1. Исключение по именам файлов устаревает каждый раз, поэтому строка
теперь определяется по содержимому: боевая строка — это словарь с ключом `variant`.

Отдельно видно в матрице: `systemd` показывает FAIL, потому что на момент самотеста
`systemd_state` = `starting`, а не `running`/`degraded`. На вердикт джобы это не влияет
(он смотрит только на `boot`), но столбец врёт — разбираться отдельно.

### Иконки приложений на карточках (пункт «а» из списка Макса)

В оригинале у каждой карточки слева от заголовка стоит иконка приложения. Модель tabbox отдаёт
`icon` как QIcon, и это ровно то, что чистый QtQuick нарисовать не может: `Image.source` отвечает
`Unable to assign QIcon to QUrl` (замерено в песочнице), а `QtQuick.Controls.impl.IconImage` хочет
имя из темы значков, которого модель не публикует. QIcon умеет принимать `Kirigami.Icon`.

Проблема в том, что несуществующий QML-import убивает весь файл, в котором он написан, а этот файл —
сам переключатель. Поэтому иконка живёт в отдельном `IconBadge.qml` и подключается через `Loader`:
если `qml6-module-org-kde-kirigami` не встанет, пропадёт иконка, а не Alt+Tab. Проверено локально:
без модуля в логе три ошибки загрузки бейджа, панель при этом рисуется полностью.

Заодно установщик темы копирует **все** `contents/ui/*.qml` пакета, а не только `main.qml`, и
`qml6-module-org-kde-kirigami` добавлен в список пакетов образа.

Гейты: shell 476 passed, 1 skipped; tools 44 passed.

Первая попытка чинила это по ключу `variant` — и оказалась недостаточной: манифест сборки образа,
который тоже попадает в `results/`, несёт `variant` без `boot`, и в прогоне `33327622991` матрица
получила строку `| full | ? | FAIL |`. Строка отчёта теперь определяется так: словарь, у которого
есть и `variant`, и вердикт `boot`.

## 2026-08-30 — Meta+Tab: сетка окон в форме Task View (пункт «в»)

Прогон `iso` 33328542056 — 12/12 зелёных, фантомной строки в матрице больше нет, и в сессии
`ZALDROS-SWITCHER card icon loaded` ×2: иконки на карточках Alt+Tab доказаны живой загрузкой.

Про размытие (пункт «б») ответ отрицательный и он окончательный для QML: KWin 6.6,
`src/plugins/blur/blur.cpp` (~339) читает у внутреннего окна свойство `kwin_blur` через
`property.value<Region>()` — это внутренний C++-тип KWin (`src/core/region.h`), из QML такой объект
не создать. Акриловое размытие требует своего C++-эффекта или настоящего wayland-клиента.

Что сделано вместо этого. Task View у KWin (`src/plugins/overview/qml/Main.qml`) тянет компоненты
Plasma и Milou, то есть в сессии Zaldros не загрузится (ADR-0008). Но у tabbox есть вторая,
*альтернативная* раскладка, и это тот же механизм, который уже доказал, что рисуется:

* Из одного исходника собираются **два** пакета: `zaldros` (Alt+Tab, карточки 300×188, ряд) и
  `zaldros-grid` (Meta+Tab, карточки 420×262, не больше трёх в ряду → окна переносятся в сетку).
  Различаются только подстановки установщика: `@LAYOUT@ @CARDW@ @CARDH@ @MAXCOL@`. Правка
  переключателя — автоматически правка обеих раскладок.
* `[TabBoxAlternative] LayoutName=zaldros-grid` в kwinrc, `Walk Through Windows (Alternative)` на
  Meta+Tab в kglobalshortcutsrc. Собственное циклирование скрипта (без UI) переехало на Meta+F10,
  туда же переехала проба драйвера (`PROBE_KEYS: meta_f10`).
* Карточки масштабируются под экран: сначала из бюджета вычитаются промежутки и заголовки, потом
  миниатюры ужимаются в остаток (`fitScale`, только уменьшение). Локальный офскрин-рендер: пять
  окон при фиксированных размерах обрезали нижний ряд, а с общим масштабом дают сетку 3×2 целиком.
* `ui-drive.py` водит Meta+Tab тем же трёхкадровым способом, что и Alt+Tab (`task_view-before/
  held/after`), в отчёте появилась колонка `task_view`.

Честное ограничение: tabbox закрывается при отпускании модификатора, поэтому это *подсматривание*
в стиле Task View, а не залипающий экран задач с виртуальными столами — тот требует настоящего
эффекта на C++.

Гейты: shell 480 passed, 1 skipped; tools 44 passed.
