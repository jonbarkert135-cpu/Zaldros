...
иначе прочерк); в шаге «Result matrix» появился `set -o pipefail` — в `.github/workflows/iso.yml`
и в копии `docs/ci/iso.yml`; `tests/test_boot_report.py` проверяет матрицу на форме, которую
реально пишет гость, код возврата на упавшей загрузке и сам workflow на `pipefail`; `BUILD.md`
...
`build/iso/report.py`, workflow и документы.

**Чего нет.** Локального ISO и локальной загрузки. Исправленная матрица ещё ни разу не
